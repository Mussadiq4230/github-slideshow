    ...
    <div class="container">
      ...
      <aside id="column-left" class="col-sm-3 hidden-xs">
            ...
            <div class="box-category border_bottom" >
              <ul id="cat_accordion">
                <li><a href="/shop{{$offer_type_filter}}" style="font-weight: <?php echo "All Categories" == $selected ? "bold": "normal"; ?>">All Categories</a></li>
                @foreach($pc_categories as $cat)

                <li 
                
                ><a href="<?php if($cat->parent_category_name == 'Kids') { echo '#'; }else{ echo '/shop/'.$cat->parent_category_slug; }?>{{$offer_type_filter}}"
                  ...
                  <ul id="{{$cat->parent_category_slug}}" class="<?php if($cat->parent_category_name !='Kids') {echo 'term-list';} ?>">
                   @if($cat->sub_parent_categories)
                     ...
                     <li ><a  href="/shop/{{$sub_parent_category->parent_category_slug}}{{$offer_type_filter}}"
                      ...
                      <ul id="{{$sub_parent_category->parent_category_slug}}" class="term-list">
                        @foreach($sub_parent_category->our_categories as $category)
                          <li class="term-item" >
                            ...
                            href="/shop/{{$sub_parent_category->parent_category_slug}}/{{$category->category_slug}}{{$offer_type_filter}}">{{$category->category_name}}</a> 
                            ...
                            <ul id="{{$category->category_slug}}">
                              @if(isset($category->sub_categories))
                                @foreach($category->sub_categories as $sub_cat)
                                  <li><a href="/shop/{{$sub_parent_category->parent_category_slug}}/{{$category->category_slug}}/{{$sub_cat->category_slug}}{{$offer_type_filter}}">{{$sub_cat->category_name}} </a></li>
                                @endforeach
                              @endif
                            </ul>
                          </li>
                   ...
                   @else
                     @foreach($cat->our_categories as $category)
                        <li class="term-item"><a
                            style="font-weight: <?php echo $category->category_name == $selected ? "bold": "normal"; ?>"
                         href="/shop/{{$cat->parent_category_slug}}/{{$category->category_slug}}{{$offer_type_filter}}">{{$category->category_name}}</a> 
                          @if(!empty($category->sub_categories))<span class="down"></span> @endif
                          <ul>
                            @foreach($category->sub_categories as $sub_cat)
                            <li><a 
                              href="/shop/{{$cat->parent_category_slug}}/{{$category->category_slug}}/{{$sub_cat->category_slug}}{{$offer_type_filter}}">{{$sub_cat->category_name}} </a></li>
                            @endforeach
                          </ul>
                        </li>
                      @endforeach
        ...
        <?php 
          ...
          ?>
             ...
             <?php 
            if(isset($all_counts['feautre']) && !(isset($all_counts['feature'][0]) && count($all_counts['feature']) == 1 && $all_counts['feature'][0]->count_name == "Unspecified" && $all_counts['feature'][0]->total_count <1)){
              ?>
        ...
        <div id="content" class="col-sm-9">
          <h1 class="title">
            @if($is_today)
              {{"Today Sales"}}
            @else
              {{$selected_offer->offer_type_name}}
            @endif</h1>
          <h3 class="subtitle">Other Offers</h3>
           <div class="category-list-thumb row">

            @foreach($other_counts['offer_type'] as $offer)
            
              <div class="col-lg-2 col-md-2 col-sm-2 col-xs-4">  
                <a href="{{route('shop.offerProducts',$offer->count_slug)}}"> {{$offer->count_name}}({{$offer->total_count}})
                </a> 
              </div>
            @endforeach
              ...
              <div class="product-layout product-list col-xs-12">
                <div class="product-thumb">
                  ...
                  <div>
                    ...
                    <div class="caption">
                      <h4>
                        <a href="#" onclick="redirectProduct('{{$product->id}}','no')">{{$product->name}}</a></h4>
